...
namespace BachMaiCR.DataAccess.Repository
{
  public class ConfigHolidaysRepository : EFRepository<CONFIG_HOLIDAYS>, IConfigHolidaysRepository, IRepository<CONFIG_HOLIDAYS>
  {
    ...
    {
      int num = this.DbSet.AsNoTracking().Where((obj => obj.DOCTORS_ID == idDoctor && SqlFunctions.DateDiff("dd", obj.DATE_BEGIN, dateCheck) >= (int?) 0 && SqlFunctions.DateDiff("dd", obj.DATE_END, dateCheck) <= (int?) 0)).ToList<CONFIG_HOLIDAYS>().Count<CONFIG_HOLIDAYS>();
      bool flag = false;
      if (num > 0)
        return true;
      return flag;
...
label_20:
        if (num3 == 0)
          source = source.Where((t => t.HOLIDAYS_ID == (int?) entity.SearchHolidayId.Value));
        ...
        return source.OrderBy((t => t.DOCTOR.DOCTOR_NAME)).OrderBy<CONFIG_HOLIDAYS, DateTime?>((Expression<Func<CONFIG_HOLIDAYS, DateTime?>>) (t => t.DATE_BEGIN)).Paginate<CONFIG_HOLIDAYS>(page, size, 0);
      }
      catch
      {
