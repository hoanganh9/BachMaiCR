...
namespace BachMaiCR.DataAccess.Repository
{
  public class ConfigHolidaysRepository : EFRepository<CONFIG_HOLIDAYS>, IConfigHolidaysRepository, IRepository<CONFIG_HOLIDAYS>
  {
    ...
    {
      int num = this.DbSet.AsNoTracking().Where((obj => obj.DOCTORS_ID == idDoctor && SqlFunctions.DateDiff("dd", obj.DATE_BEGIN, dateCheck) >= (int?) 0 && SqlFunctions.DateDiff("dd", obj.DATE_END, dateCheck) <= (int?) 0)).ToList().Count<CONFIG_HOLIDAYS>();
      bool flag = false;
      if (num > 0)
        return true;
      return flag;
